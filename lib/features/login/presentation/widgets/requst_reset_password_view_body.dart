    ...
    return SafeArea(
      child: Scaffold(
        appBar: PreferredSize(
          ...
          child: CustomAppBar(
            text: S.of(context).forgetPasswoerd,
            needArrow: false,
            textStyle: const TextStyle(fontFamily: "GE SS TWO",fontWeight: FontWeight.bold),
            
          ),
        ),
        body: BlocConsumer<RequsetCubitCubit, RequsetCubitState>(
          listener: (context, state) {
            if (state is ForgetPasswordSuccess) {
              ScaffoldMessenger.of(context).showSnackBar(
                CustomSnackBar.success(
                  context: context,
                  message: S.of(context).verificationsend,
                ),
              );
